...
export const useProfilePhotoCache = (userId?: string) => {
  ...
  const uploadPhoto = useCallback(async (file: File) => {
    ...
    try {
      ...
      // Upload
      const response = await userService.uploadProfilePhotoBlob(
        base64,
        mimeType
      );
  ...
  const checkIfCacheNeedsRefresh = useCallback(async () => {
    try {
      ...
      const serverMetadata = serverMetadataResp?.data as
        ...
        | undefined;
      const localMetadata = metadata as Record<string, unknown> | null;

      const serverLast = serverMetadata?.["lastUpdated"];
      const localLast = localMetadata?.["lastUpdated"];
